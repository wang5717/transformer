...
class TrackingBaseModel(nn.Module):

    def __init__(self, object_detection_model, track_query_false_positive_prob: float = 0.0,
                 track_query_false_negative_prob: float = 0.0, matcher: HungarianMatcher = None,
    ...
    def forward(self, samples: NestedTensor, targets: list = None, prev_features=None):
            ...
            # if True:
                ...
                with backprop_context():
                    if 'prev_prev_image' in targets[0]:
                        ...
                        # PREV PREV
                        prev_prev_out, _, prev_prev_features, _, _ = self._object_detection_model.forward([t['prev_prev_image'] for t in targets])
                        ...
                        # PREV
                        prev_out, _, prev_features, _, _ = self._object_detection_model.forward(
                            [t['prev_image'] for t in targets],
                            prev_targets,
                            prev_prev_features)
                    else:
                        prev_out, _, prev_features, _, _ = self._object_detection_model.forward([t['prev_image'] for t in targets])
        ...
        out, targets, features, memory, hs  = self._object_detection_model.forward(samples, targets, prev_features)
